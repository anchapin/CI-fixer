...
- **Adaptive Runtime Management:** Context-sensitive environment detection (Bun vs. Node) with **automated runtime provisioning** (intelligent PATH management, loop prevention), **persistent environment state**, and runtime-aware test execution.
- **Automated Root Cause Analysis:** Employs intelligent log parsing, robust file discovery (including fuzzy search, git history tracking, **fuzzy file recovery**, and a dedicated **Grounding Layer** for path verification and auto-correction), and AST-based code analysis to pinpoint the exact source of failures.
- **Strict Language Scoping:** Uses a hybrid keyword/manifest detection engine to enforce language boundaries (JS/TS, Python, Go), ensuring the agent prioritizes relevant files and tools for the detected technology stack and avoids incorrect cross-language fixes.
...
- **Autonomous Dependency Resolution:** Resolves complex Python dependency conflicts (e.g., `crewai`, `pydantic`) by combining advanced package manager diagnostics (`pip --dry-run --report`), automated resolution tooling (`pip-compile`), and LLM-driven constraint relaxation to ensure environment stability.
- **Robust Reproduction Inference:** Automatically infers missing reproduction commands through multi-layered analysis of GitHub workflows (with **targeted parsing of failing workflows** and **LLM-assisted pinpointing of specific run commands**), project signatures (e.g., package.json, requirements.txt), build tools (e.g., Makefile), and LLM-based repository analysis. Includes a **Safe Scan** fallback for deep-search of test-like files and performs **sandbox-based dry-run validation** to ensure command viability before verification.
...
- **Loop Detection & Mitigation**: Identifies when the agent is repeating failed strategies (using a **hash-based Loop Detector**) and uses **telemetry-driven advice injection** to guide the agent out of repetitive error states.
