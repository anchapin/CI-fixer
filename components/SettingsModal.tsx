...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, onSave, currentConfig, onExportLogs }) => {
    ...
    useEffect(() => {
        if (isOpen && currentConfig) {
            setFormData({
                ...
                e2bApiKey: currentConfig.e2bApiKey || process.env.E2B_API_KEY || '',
    ...
    const handleTestE2B = async () => {
        ...
        // Note: Removed auto-switch logic to prevent modal reload and dropdown reset
        // Users will see the network error message and can manually switch if needed
                            ...
                            <div className="space-y-3">
                                ...
                                {formData.devEnv === 'e2b' && formData.executionBackend === 'e2b' && (
                                    <div className="space-y-2 animate-[fadeIn_0.2s_ease-out]">
                                        ...
                                        {e2bStatus && (
                                            <div className={`text-[10px] font-bold px-1 flex items-center gap-1 ${e2bStatus.success ? 'text-emerald-400' :
                            ...
                            <div className="space-y-2 max-h-[150px] overflow-y-auto">
                                {foundRuns.map(run => (
                                    <div key={run.id}
                                        ...
                                        className={`flex items-center p-2 rounded cursor-pointer border transition-colors ${formData.selectedRuns?.find(r => r.id === run.id)
                                            ? 'bg-rose-950/20 border-rose-900/50'
                                            : 'bg-slate-900/50 border-transparent hover:bg-slate-900'
                                            }`}
                                    >
                                        <div className={`w-4 h-4 rounded border flex items-center justify-center mr-3 ${formData.selectedRuns?.find(r => r.id === run.id)
